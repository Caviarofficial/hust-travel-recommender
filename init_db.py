...
def init_database():
    ...
    c.execute("DROP TABLE IF EXISTS item_path_mapping")

    c.execute("""
        CREATE TABLE items (
            item_id INTEGER PRIMARY KEY,
            name TEXT NOT NULL,
            distance TEXT CHECK(distance IN ('near','far')),
            energy TEXT CHECK(energy IN ('low','high')),
            category TEXT,
            cost TEXT CHECK(cost IN ('free','low','mid','high')),
            tags TEXT
        )
    """)
    ...
    c.execute("""
        CREATE TABLE item_path_mapping (
            path_code TEXT,
            item_id INTEGER,
            priority INTEGER,
    ...
    for path_code, item_ids in PATH_MAPPING.items():
        for priority, item_id in enumerate(item_ids):
            c.execute(
                "INSERT INTO item_path_mapping VALUES (?,?,?)",
                (path_code, item_id, priority)
            )

    conn.commit()
    conn.close()
    print("数据库初始化完成！已创建 survey.db")
    print(f"  - 写入 {len(ITEMS)} 个项目")
    print(f"  - 写入 {len(PATH_MAPPING)} 条路径映射")
